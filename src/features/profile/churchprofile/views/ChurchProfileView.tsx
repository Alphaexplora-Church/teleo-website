...
interface ChurchProfileViewProps {
  ...
  onServiceSelect?: (serviceName: string, church: { id: string | number; name: string }) => void;
}
  ...
  return (
    <div className="flex flex-col w-full min-h-screen bg-gray-50 relative">
      ...
      <section className="relative w-full min-h-65 shrink-0 flex flex-col justify-start gap-2 bg-navy overflow-hidden">
        ...
        {/* Top Header Row: Back Button (Left) & Church Logo Avatar (Right) */}
        <div className="relative z-10 px-4 pt-4 flex items-center justify-between">
          ...
          {/* Circular Church Logo Icon */}
          <div className="size-9 rounded-full overflow-hidden border border-white/30 shadow-md bg-white/10 backdrop-blur-xs shrink-0 flex items-center justify-center">
      ...
      {/* ═══════════════════════════════════════════════════════════
          3. Content Area
   ═══════════════════════════════════════════════════════════ */}
      <section className="w-full max-w-4xl mx-auto px-5 pt-5 pb-6">
        {/* ── Overview Tab ──────────────────────────────────────── */}
        {activeTab === 'overview' && (
          <div className="flex flex-col gap-8">
            ...
            {church.location && (
              <div className="flex flex-col gap-3">
                ...
                {/* Actionable Address Bar */}
                <a
                  href={`https://www.google.com/maps?q=${encodeURIComponent(church.location)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`Open directions to ${church.location} in Google Maps`}
                  className="flex items-center justify-between gap-2.5 p-3.5 bg-blue-50/70 hover:bg-blue-100/70 border border-blue-200/80 rounded-xl text-[#2563EB] hover:text-blue-700 transition-all group outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="shrink-0 text-[#2563EB]">
                      <LocationIcon />
                    </div>
                    <span className="text-sm font-semibold leading-relaxed truncate group-hover:underline">
                      {church.location}
                    </span>
                  </div>
                  <div className="shrink-0 text-[#2563EB] transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5">
                    <ExternalLinkIcon />
                  </div>
                </a>
              </div>
        ...
        {activeTab === 'announcements' && (
          <div className="-mx-5 flex flex-col">
            ...
            {(!church.announcements || church.announcements.length === 0) && (
              <p className="text-center text-gray-500 py-8 px-5">No announcements at this time.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'events' && (
          <div className="-mx-5 flex flex-col">
            ...
            {(!church.events || church.events.length === 0) && (
              <p className="text-center text-gray-500 py-8 px-5">No upcoming events.</p>
            )}
          </div>
        )}
            ...
            <div className="grid grid-cols-2 gap-3">
              {church.services?.map((svc) => (
                <div
                  key={svc.id}
                  onClick={() =>
                    onServiceSelect?.(svc.name, { id: church.id, name: church.name })
                  }
                  className="relative rounded-2xl overflow-hidden shadow-sm h-48 flex flex-col justify-end bg-navy group cursor-pointer active:scale-95 transition-transform"
                >
                  {/* Background Image */}
                  {svc.image && (
                    <img
