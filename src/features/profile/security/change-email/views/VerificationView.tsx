// features/profile/security/change-email/views/VerificationView.tsx
// View layer — dumb UI only. NO useState (except local), NO useEffect, NO API calls.
// All state and logic is delegated to useVerificationViewModel.
//
// Receives the target email via props (passed from AppShell after "Send Code" succeeds).
...
interface VerificationViewProps {
  /** The email address the OTP was sent to. Passed in by the shell. */
  targetEmail: string;
...
const VerificationView: React.FC<VerificationViewProps> = ({ targetEmail, onSuccess }) => {
  const vm = useVerificationViewModel({ targetEmail, onSuccess });

  const isBusy = vm.isVerifying || vm.isResending;

  // Backspace auto-retreat: focus previous cell when current cell is empty.
  // This is pure focus/DOM logic — belongs in the View, not the ViewModel.
        ...
        <div className="w-full rounded-[20px] bg-black/5 flex flex-col items-center gap-6 px-6 pt-10 pb-8">
          ...
          <div className="flex flex-col items-center gap-2 text-center">
            ...
            <p className="text-black/60 text-sm font-normal font-sans leading-5">
              ...
              <span className="text-black font-medium break-all">{targetEmail}</span>
            </p>
          </div>

          {/* ── Feedback banners ───────────────────────────────── */}
          {vm.verifySuccess && (
            <FeedbackBanner type="success" message="Email changed successfully! 🎉" />
          )}
          {vm.verifyError && (
            <FeedbackBanner
              type="error"
