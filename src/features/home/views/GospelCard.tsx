...
interface GospelCardProps {
  ...
  onEventOpen: (postId: string) => void;
}

const GospelCard: React.FC<GospelCardProps> = ({
  slides,
  activeIndex,
  dailyGospel,
  isGospelLoading,
  ...
  <section
    className="relative h-[190px] overflow-hidden rounded-xl bg-[#001739] text-white shadow-sm"
    aria-roledescription="carousel"
    aria-label="Featured church updates"
  >
        ...
        {slide.kind === 'gospel' ? (
          <>
            ...
            <div className="relative flex h-full flex-col p-4 drop-shadow-[0_1px_3px_rgba(0,0,0,0.75)]">
              <p className="flex items-center gap-2 text-[10px] font-semibold uppercase">
                <Icon size={20}>
                  <rect x="3" y="4" width="18" height="16" rx="2" />
                  <path d="M7 8h4v8H7zM14 8h3M14 12h3" />
                </Icon>
                Gospel of the day
              </p>
              <h1 className="mt-2 text-[22px] font-bold tracking-tight">
                {isGospelLoading
                  ? "Loading today's Gospel"
                  : dailyGospel?.reference ?? 'Gospel unavailable'}
              </h1>
              <p className="mt-1 pr-2 text-[11px] font-normal leading-[18px] text-white/95">
                {isGospelLoading
                  ? 'Preparing the daily reading...'
                  : dailyGospel?.content ??
                    gospelError ??
          ...
          <>
            <button
              type="button"
              onClick={() => slide.postId && onEventOpen(slide.postId)}
              aria-label={`Open ${slide.title ?? 'event'}`}
            ...
            <div className="relative flex h-full flex-col p-4 drop-shadow-[0_1px_3px_rgba(0,0,0,0.75)]">
              <p className="flex items-center gap-2 text-[10px] font-semibold uppercase">
                <span className="flex h-7 w-7 items-center justify-center rounded bg-[#DBE0E4] text-[#042C58]">
                  <Icon size={18}>
              ...
              <h1 className="mt-1 line-clamp-3 max-w-[285px] font-serif text-[25px] font-bold leading-[1.05]">
                {slide.title}
              </h1>
              <p className="mt-2 max-w-[250px] truncate text-[12px] font-semibold text-[#3399FF]">
                {slide.location}
              </p>
              <p className="mt-1 flex items-center gap-2 text-[11px]">
                <span className="flex h-4 w-4 items-center justify-center rounded-full bg-white">
                  <img src={timeIcon} alt="" className="h-3 w-3 brightness-0" />
                </span>
                {slide.time}
              </p>
              <div className="absolute right-4 top-5 flex h-[74px] w-[64px] flex-col items-center justify-center rounded-[22px] border border-[#FFAF00] bg-[#001739]/30 leading-none backdrop-blur-sm">
                ...
                <strong className="mt-1 text-[31px]">{slide.day}</strong>
              </div>
            </div>
          </>
        )}
      ...
      >
        {slides.map((slide, index) => (
          <button
            ...
            aria-label={
              slide.kind === 'gospel'
                ? 'Show Gospel of the Day'
                : `Show ${slide.title}`
            }
            onClick={() => onSlideChange(index)}
            className={`h-2 rounded-full transition-all ${
              activeIndex === index ? 'w-7 bg-[#3399FF]' : 'w-2 bg-white/80'
            }`}
          />
        ))}
      </div>
